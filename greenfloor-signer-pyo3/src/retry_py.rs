...
fn moderate_retry_sleep_seconds_py(
    attempt: u32,
    current_sleep: f64,
    rate_limit_wait: Option<f64>,
) -> f64 {
    moderate_retry_sleep_seconds(attempt, current_sleep, rate_limit_wait)
}
...
#[pyfunction]
#[pyo3(name = "poll_exponential_next_sleep")]
fn poll_exponential_next_sleep_py(
    elapsed_seconds: i64,
    timeout_seconds: i64,
    current_sleep: f64,
    initial_sleep: f64,
...
pub fn register(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(poll_exponential_next_sleep_py, m)?)?;
    Ok(())
}
