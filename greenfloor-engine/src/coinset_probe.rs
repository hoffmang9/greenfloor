...
use crate::vault::members::{hex_to_bytes32, singleton_member_puzzle_hash_hex_from_launcher_id};
...
impl NamesCapability {
    ...
    pub fn from_attempts(sample_name: String, all: ProbeAttempt, range: ProbeAttempt) -> Self {
        Self {
            sample_name: Some(sample_name),
            all_supported: Some(all.supported),
            all_error: all.error,
            all_count: all.count,
            range_supported: Some(range.supported),
            range_error: range.error,
            range_count: range.count,
        }
    }
...
fn ensure_program_config_valid(program_config: &str) -> SignerResult<()> {
    let path = program_config_path(program_config);
    if path.exists() {
        let _ = load_program_config(&path)?;
    }
    Ok(())
}
...
async fn probe_names(
    ...
    let (range, _) =
        ProbeAttempt::run(|| client.by_names(&names, true, Some(start_height), Some(end_height)))
            .await;
    NamesCapability::from_attempts(sample_name.to_string(), all, range)
}
...
pub async fn build_coinset_probe_report(args: CoinsetProbeCliArgs) -> SignerResult<ProbeReport> {
    ensure_program_config_valid(&args.program_config)?;

    let program_config_path =
        optional_trimmed(&args.program_config).map(|value| expand_home(Path::new(&value)));
    let resolved = resolve_launcher_id(&ResolveLauncherIdParams {
        ...
        program_config: program_config_path.as_deref(),
    })?;

    let p2_hash =
        singleton_member_puzzle_hash_hex_from_launcher_id(&resolved.launcher_id, args.nonce)?;
    let p2_coinset_hex = to_coinset_hex(hex_to_bytes32(&p2_hash)?.as_ref());
