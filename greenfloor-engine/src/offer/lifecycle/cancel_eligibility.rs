...
/// Drop Dexie-open ids whose [`SQLite`] state is already `cancel_submitted`.
#[must_use]
pub fn filter_out_cancel_pending_offer_ids(
    offer_ids: &[String],
...
/// Whether a persisted state string should stay on the daemon reconcile watchlist.
#[must_use]
pub fn is_watched_for_reconcile(state: &str) -> bool {
    ReconcileState::parse(state).is_ok_and(|parsed| parsed.is_watched_for_reconcile())
}
...
mod tests {
    ...
    #[test]
    fn filter_out_cancel_pending_offer_ids_skips_cancel_submitted() {
        let open_ids = vec!["o1".to_string(), "o2".to_string()];
        let db_rows = vec![row("o1", "open"), row("o2", "cancel_submitted")];
        assert_eq!(
            filter_out_cancel_pending_offer_ids(&open_ids, &db_rows),
            vec!["o1".to_string()]
        );
    }
    ...
    #[test]
    fn is_watched_for_reconcile_includes_cancel_submitted() {
        assert!(is_watched_for_reconcile("cancel_submitted"));
        assert!(is_watched_for_reconcile("open"));
        assert!(is_watched_for_reconcile("unknown_orphaned"));
        assert!(!is_watched_for_reconcile("cancelled"));
    }
}
