...
use chia_puzzles::{DELEGATED_PUZZLE_FEEDER_HASH, ONE_OF_N_HASH, SETTLEMENT_PAYMENT_HASH};
use chia_sdk_driver::{
    Cat, CatSpend, InnerPuzzleSpend, MipsSpend, Offer, Puzzle, Spend, SpendContext, Vault,
};
use chia_sdk_types::puzzles::{
    DelegatedPuzzleFeederArgs, DelegatedPuzzleFeederSolution, IndexWrapperArgs, OneOfNArgs,
    OneOfNSolution, SingletonMember, SingletonMemberSolution, INDEX_WRAPPER_HASH,
};
...
impl PresplitOfferBinding {
    ...
    pub(crate) fn from_presplit_input_spend(
        ...
        let fixed_conditions_tree_hash =
            presplit_fixed_conditions_tree_hash_from_input(launcher_id, chain_cat, spend_bundle)?;
        Ok(Self {
