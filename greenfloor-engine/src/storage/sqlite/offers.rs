...
mod list_offer_states_tests {
    ...
    #[test]
    fn list_offer_state_details_includes_cancel_tracking_columns() {
        let dir = tempfile::tempdir().expect("tempdir");
        let store = SqliteStore::open(&dir.path().join("state.db")).expect("open");
        ...
        let rows = store.list_offer_state_details("m1", 10).expect("details");
        assert_eq!(rows.len(), 1);
        assert_eq!(rows[0].offer_id, "offer-cancel");
    ...
    #[test]
    fn offer_state_for_id_returns_none_for_missing_offer() {
        let dir = tempfile::tempdir().expect("tempdir");
        let store = SqliteStore::open(&dir.path().join("state.db")).expect("open");
        assert!(store
            .offer_state_for_id("missing-offer")
            .expect("lookup")
            .is_none());
    }
