...
pub(super) enum NormalizeOutcome<'a> {
    Ready(NormalizedReservationLeaseRequest<'a>),
    Rejected(String),
}

impl NormalizedReservationLeaseRequest<'_> {
    pub(super) fn normalize<'a>(
        request: &'a OfferReservationLeaseRequest<'a>,
    ) -> SignerResult<NormalizeOutcome<'a>> {
        ...
        validate_reservation_id_and_lease(reservation_id, lease_seconds)?;
        let normalized_requests = positive_asset_amounts(requested_amounts);
        if normalized_requests.is_empty() {
            return Ok(NormalizeOutcome::Rejected(
                "reservation_empty_request".to_string(),
            ));
        }
        let normalized_available = positive_asset_amounts(available_amounts);
        let (now_iso, expires_at_iso) =
            lease_window_iso(now.unwrap_or_else(Utc::now), lease_seconds);
        Ok(NormalizeOutcome::Ready(NormalizedReservationLeaseRequest {
            reservation_id,
            market_id,
            wallet_id,
            normalized_requests,
...
pub(super) fn first_insufficient_asset_error(
...
mod tests {
    ...
    fn normalize_rejects_empty_request() {
        ...
        let outcome = NormalizedReservationLeaseRequest::normalize(&request).expect("normalize");

        let NormalizeOutcome::Rejected(reason) = outcome else {
            panic!("expected empty-request rejection");
        };
        assert_eq!(reason, "reservation_empty_request");
    }
    ...
    #[test]
    fn first_insufficient_asset_error_none_when_capacity_sufficient() {
        ...
        assert!(first_insufficient_asset_error(&requests, &available, &reserved).is_none());
    }

    #[test]
    fn first_insufficient_asset_error_reports_first_shortfall() {
        let mut requests = BTreeMap::default();
        ...
        let err = first_insufficient_asset_error(&requests, &available, &BTreeMap::default())
            .expect("shortfall");

        assert!(err.contains("reservation_insufficient_asset"));
        assert!(err.contains("available=50"));
        assert!(err.contains("needed=100"));
    }

    #[test]
    fn first_insufficient_asset_error_accounts_for_existing_reservations() {
        let mut requests = BTreeMap::default();
        ...
        let err =
            first_insufficient_asset_error(&requests, &available, &reserved).expect("shortfall");
        assert!(err.contains("reserved=50"));
        assert!(err.contains("needed=60"));
    }
}
