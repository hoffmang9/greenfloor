...
#[cfg(test)]
mod tests {
    use greenfloor_engine::offer::types::{CreateOfferResult, OfferExecutionMode};
    use greenfloor_engine::vault::{MixedSplitResult, VaultContext};

    use super::{print_create_offer_result, print_mixed_split_result, print_vault_info};

    fn sample_vault_context() -> VaultContext {
        VaultContext {
            network: "mainnet".to_string(),
            launcher_id: "aa".repeat(64),
            inner_puzzle_hash: "bb".repeat(64),
            p2_singleton_message_hash: "cc".repeat(64),
            custody_hash: "dd".repeat(64),
            recovery_hash: "ee".repeat(64),
            custody_threshold: 1,
            recovery_threshold: 1,
            recovery_clawback_timelock: 3600,
            secp256r1_custody_keys: vec!["ff".repeat(64)],
            kms_public_key_hex: "11".repeat(64),
            kms_custody_key_match: true,
        }
    }

    #[test]
    fn print_vault_info_emits_human_readable_fields() {
        print_vault_info(&sample_vault_context());
    }

    #[test]
    fn print_mixed_split_result_supports_json_and_text_modes() {
        let result = MixedSplitResult {
            spend_bundle_hex: "abc123".to_string(),
        ...
        print_mixed_split_result(&result, true).expect("json print");
        print_mixed_split_result(&result, false).expect("text print");
    }

    #[test]
    fn print_create_offer_result_supports_json_and_text_modes() {
        let result = CreateOfferResult {
            offer: "offer1qqq".to_string(),
            ...
            presplit_cancel_fields: None,
        };
        print_create_offer_result(&result, true).expect("json print");
        print_create_offer_result(&result, false).expect("text print");
    }
}
