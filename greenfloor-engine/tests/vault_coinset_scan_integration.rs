...
async fn vault_coinset_scan_discovers_xch_coin_via_hint_lookup() {
    ...
    let request = scan_request(&server, &launcher, dir.path(), 100);
...
#[tokio::test]
async fn vault_coinset_scan_keeps_coins_when_verify_returns_empty() {
    ...
    let dir = tempfile::tempdir().expect("tempdir");
    let request = scan_request(&server, &launcher, dir.path(), 50);

    let result = ScanState::run(request).await.expect("scan should succeed");
    assert_eq!(result.count, 1);
    let verification = result.name_verification.expect("verification summary");
    assert!(!verification.applied);
    assert_eq!(verification.pre_verify_count, 1);
    assert_eq!(verification.verified_count, None);
    assert_eq!(verification.dropped_unverified_count, 0);
}
...
async fn vault_coinset_scan_drops_unverified_coins() {
    ...
    let coin_id = coin_id_from_record(&coin_record);
    let coin_name = to_coinset_hex(hex_to_bytes32(&coin_id).expect("coin id bytes").as_ref());
    let other_coin_record = json!({
        "coin": {
            "parent_coin_info": "66".repeat(32),
            "puzzle_hash": "77".repeat(32),
            "amount": 1,
        },
        "confirmed_block_index": 2,
        "spent_block_index": 0,
    ...
    let verify_body = json!({
        "success": true,
        "coin_records": [other_coin_record],
    })
    .to_string();
