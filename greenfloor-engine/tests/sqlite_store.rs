...
fn get_tx_signal_state_dedupes_and_ignores_empty_ids() {
    ...
    assert_eq!(
        store
            .observe_mempool_tx_ids(&["tx-a".to_string(), "tx-b".to_string()])
            .expect("observe"),
        2
    );
    assert_eq!(
        store
            .confirm_tx_ids(&["tx-a".to_string()])
            .expect("confirm"),
        1
    );
    let state = store
        .get_tx_signal_state(&[
            ...
            "tx-a".to_string(),
            "tx-b".to_string(),
            "tx-missing".to_string(),
        ])
        .expect("state");
    assert_eq!(state.len(), 2);
    assert!(state["tx-a"].mempool_observed_at.is_some());
    assert!(state["tx-a"].tx_block_confirmed_at.is_some());
    assert!(state["tx-b"].mempool_observed_at.is_some());
    assert!(state["tx-b"].tx_block_confirmed_at.is_none());
}
