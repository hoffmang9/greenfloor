...
- **Bootstrap mixed-output planner:** `plan_bootstrap_mixed_outputs` in `greenfloor-signer/src/offer/bootstrap.rs` returns `BootstrapPlanOutcome` (`ready` / `needs_split` / `cannot_fund` / `invalid_ladder` / `invalid_coins`); PyO3 in `offer_bootstrap_py.rs` requires `PlannerLadderRow` + `BootstrapCoin`; early/executed phase shaping (`bootstrap_early_phase`, `bootstrap_executed_phase`) is Rust-backed; runtime returns typed `BootstrapPhaseResult` / `BootstrapPreflight`.
