...
- **Step 16 — Coin-op gates (`greenfloor-signer/src/coin_ops/gate.rs`, `wallet_coin.rs`):** `evaluate_coin_split_gate`, `evaluate_coin_combine_gate`, `coin_op_should_stop`, `is_spendable_wallet_coin`; bridge converts PyO3 gate results to `SplitDenominationReadiness` / `CombineDenominationReadiness`; `runtime/coin_ops/readiness.py` owns iteration payload + target-based evaluation; dict JSON via `to_payload()` only.
