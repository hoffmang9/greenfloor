...
7. `offers-reconcile` — refresh offer states using venue payloads plus Coinset tx-signal state (webhook/mempool) and flag orphaned/unknown.
...
- `offers-reconcile` derives taker confirmation from Coinset tx-block webhook/mempool state first, and uses venue status as fallback context when Coinset evidence is unavailable.
...
- [x] H2: Move offer lifecycle/taker reconciliation to Coinset webhook-first signal routing.
  - `offers-reconcile` and daemon lifecycle transitions now consume `tx_signal_state` first (Coinset webhook + mempool ingestion) and treat venue status as fallback diagnostics.
  - Taker detection now emits `coinset_tx_block_webhook` on Coinset-confirmed transitions and includes `signal_source` + Coinset tx-id evidence in reconciliation/audit payloads.
