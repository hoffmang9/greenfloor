...
- **Shared vault mixed-split submit:** crate-private
  `vault::submit_vault_cat_mixed_split` with `CatSelection` (selection, client,
  broadcast). Public `build_and_optionally_broadcast_vault_cat_mixed_split` is the
