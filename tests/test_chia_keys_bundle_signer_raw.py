...
from greenfloor.cli.chia_keys_bundle_signer_raw import execute_payload
...
def test_raw_returns_engine_request_when_engine_missing(tmp_path: Path) -> None:
    keyring = tmp_path / "keyring.yaml"
    keyring.write_text("version: 1\n", encoding="utf-8")
    old = os.getenv("GREENFLOOR_CHIA_KEYS_RAW_ENGINE_CMD")
    try:
        os.environ.pop("GREENFLOOR_CHIA_KEYS_RAW_ENGINE_CMD", None)
        ...
        if old is None:
            ...
            os.environ["GREENFLOOR_CHIA_KEYS_RAW_ENGINE_CMD"] = old


def test_raw_delegates_to_engine(tmp_path: Path) -> None:
    keyring = tmp_path / "keyring.yaml"
    keyring.write_text("version: 1\n", encoding="utf-8")
    ...
    old = os.getenv("GREENFLOOR_CHIA_KEYS_RAW_ENGINE_CMD")
    try:
        os.environ["GREENFLOOR_CHIA_KEYS_RAW_ENGINE_CMD"] = f"{sys.executable} {script}"
        out = execute_payload(_payload(str(keyring)))
