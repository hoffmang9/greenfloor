...
from greenfloor.cli.chia_keys_raw_engine_sign_impl import execute_payload
...
def test_impl_returns_sign_tx_request_when_cmd_missing(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert out["sign_tx_request"]["additions"][0]["amount"] == 10
    finally:
        if old is None:
            os.environ.pop("GREENFLOOR_CHIA_KEYS_RAW_ENGINE_SIGN_IMPL_SDK_SUBMIT_CMD", None)
        else:
...
def test_impl_delegates_to_cmd(tmp_path: Path) -> None:
    keyring = tmp_path / "keyring.yaml"
    keyring.write_text("version: 1\n", encoding="utf-8")
    script = tmp_path / "sdk_submit.py"
    script.write_text(
        (
            "import json,sys\n"
