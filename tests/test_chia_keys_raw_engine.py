...
from greenfloor.cli.chia_keys_raw_engine import execute_payload
...
def test_engine_returns_sign_job_when_sign_cmd_missing(tmp_path: Path) -> None:
    keyring = tmp_path / "keyring.yaml"
    keyring.write_text("version: 1\n", encoding="utf-8")
    old = os.getenv("GREENFLOOR_CHIA_KEYS_RAW_ENGINE_SIGN_CMD")
    try:
        os.environ.pop("GREENFLOOR_CHIA_KEYS_RAW_ENGINE_SIGN_CMD", None)
        ...
        if old is None:
            ...
            os.environ["GREENFLOOR_CHIA_KEYS_RAW_ENGINE_SIGN_CMD"] = old


def test_engine_delegates_to_sign_cmd(tmp_path: Path) -> None:
    keyring = tmp_path / "keyring.yaml"
    keyring.write_text("version: 1\n", encoding="utf-8")
    ...
    old = os.getenv("GREENFLOOR_CHIA_KEYS_RAW_ENGINE_SIGN_CMD")
    try:
        os.environ["GREENFLOOR_CHIA_KEYS_RAW_ENGINE_SIGN_CMD"] = f"{sys.executable} {script}"
        out = execute_payload(_payload(str(keyring)))
