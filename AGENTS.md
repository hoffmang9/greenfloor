...
- `[MUST]` `greenfloor/* adapters`: side effects only (network, filesystem, wallet). Operator signing/execution is in-process Rust (`greenfloor-engine`). Script Coinset reads and mutations shell out to `greenfloor-engine coinset {post,push-tx}` via `greenfloor.adapters.coinset_engine`.
