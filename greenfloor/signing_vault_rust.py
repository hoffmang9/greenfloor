"""Vault KMS offer and mixed-split paths via greenfloor-signer (Rust)."""
...
def is_vault_kms_payload(payload: dict[str, Any]) -> bool:
    return bool(str(payload.get("cloud_wallet_kms_key_id", "")).strip())


def program_config_path_from_payload(payload: dict[str, Any]) -> str | None:
    return rust_signer.program_config_path_from_payload(payload)
...
def build_vault_offer_via_rust_signer(payload: dict[str, Any]) -> tuple[str | None, str | None]:
    ...
    try:
        result = rust_signer.build_vault_cat_offer(
            program_path,
...
def sign_and_broadcast_vault_mixed_split_via_rust(
    ...
    try:
        result = rust_signer.build_mixed_split(program_path, request)
    except ImportError as exc:
