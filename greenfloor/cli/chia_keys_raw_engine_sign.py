...
def execute_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    # Optional direct implementation hook for real spend-bundle construction/signing.
    impl_cmd = os.getenv("GREENFLOOR_CHIA_KEYS_RAW_ENGINE_SIGN_IMPL_CMD", "").strip()
    if impl_cmd:
        try:
            completed = subprocess.run(
                shlex.split(impl_cmd),
                input=json.dumps(sign_job),
                capture_output=True,
                check=False,
                text=True,
                timeout=120,
    ...
    try:
        ...
        return {"status": "skipped", "reason": f"sign_impl_spawn_error:{exc}", "operation_id": None}
    if completed.returncode != 0:
        err = completed.stderr.strip() or completed.stdout.strip() or "unknown_error"
        return {"status": "skipped", "reason": f"sign_impl_failed:{err}", "operation_id": None}
    try:
    ...
    if spend_bundle_hex is None and status == "skipped":
        ...
        if body.get("sign_tx_request") is not None:
            result["sign_tx_request"] = body.get("sign_tx_request")
        return result
    if spend_bundle_hex is None:
        return {
            "status": "skipped",
