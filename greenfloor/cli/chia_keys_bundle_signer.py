...
def execute_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    raw_cmd = os.getenv("GREENFLOOR_CHIA_KEYS_BUNDLE_SIGNER_RAW_CMD", "").strip()
    if not raw_cmd:
        raw_cmd = f"{sys.executable} -m greenfloor.cli.chia_keys_bundle_signer_raw"

    try:
        completed = subprocess.run(
            shlex.split(raw_cmd),
            input=json.dumps(raw_sign_request),
            capture_output=True,
            check=False,
            text=True,
            timeout=120,
        ...
        return {
            "status": "skipped",
            "reason": f"bundle_signer_raw_spawn_error:{exc}",
            "operation_id": None,
        }
    if completed.returncode != 0:
        ...
        return {
            "status": "skipped",
            "reason": f"bundle_signer_raw_failed:{err}",
            "operation_id": None,
        }
    try:
        ...
        return {
            "status": "skipped",
            "reason": "bundle_signer_raw_invalid_json",
            "operation_id": None,
        }

    spend_bundle_hex = body.get("spend_bundle_hex")
    status = str(body.get("status", "executed"))
    reason = str(body.get("reason", "chia_keys_bundle_signer_success"))
    if spend_bundle_hex is None and status == "skipped":
        ...
        if body.get("engine_request") is not None:
            result["engine_request"] = body.get("engine_request")
    ...
    if spend_bundle_hex is None:
        return {
            "status": "skipped",
            "reason": "bundle_signer_raw_missing_spend_bundle_hex",
            "operation_id": None,
        }
    return {
        "status": status,
