...
def execute_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    sign_cmd = os.getenv("GREENFLOOR_CHIA_KEYS_RAW_ENGINE_SIGN_CMD", "").strip()
    if not sign_cmd:
        sign_cmd = f"{sys.executable} -m greenfloor.cli.chia_keys_raw_engine_sign"
    ...
    try:
        ...
        return {
            "status": "skipped",
            "reason": f"raw_engine_sign_spawn_error:{exc}",
            "operation_id": None,
        }
    if completed.returncode != 0:
        ...
        return {
            "status": "skipped",
            "reason": f"raw_engine_sign_failed:{err}",
            "operation_id": None,
        }
    try:
        body = json.loads(completed.stdout.strip() or "{}")
    except json.JSONDecodeError:
        return {"status": "skipped", "reason": "raw_engine_sign_invalid_json", "operation_id": None}
    spend_bundle_hex = body.get("spend_bundle_hex")
    status = str(body.get("status", "executed"))
    reason = str(body.get("reason", "raw_engine_sign_success"))
    ...
    if spend_bundle_hex is None:
        return {
            "status": "skipped",
            "reason": "raw_engine_sign_missing_spend_bundle_hex",
            "operation_id": None,
        }
    return {
        "status": status,
