...
def execute_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    signer_cmd = os.getenv("GREENFLOOR_CHIA_KEYS_BUNDLE_SIGNER_CMD", "").strip()
    if not signer_cmd:
        signer_cmd = f"{sys.executable} -m greenfloor.cli.chia_keys_bundle_signer"
    ...
    try:
        ...
        return {
            "status": "skipped",
            "reason": f"bundle_signer_spawn_error:{exc}",
            "operation_id": None,
        }
    if completed.returncode != 0:
        err = completed.stderr.strip() or completed.stdout.strip() or "unknown_error"
        return {"status": "skipped", "reason": f"bundle_signer_failed:{err}", "operation_id": None}
    try:
        body = json.loads(completed.stdout.strip() or "{}")
    except json.JSONDecodeError:
        return {"status": "skipped", "reason": "bundle_signer_invalid_json", "operation_id": None}

    spend_bundle_hex = body.get("spend_bundle_hex")
    status = str(body.get("status", "executed"))
    reason = str(body.get("reason", "chia_keys_builder_success"))
    if spend_bundle_hex is None and status == "skipped":
        ...
        if body.get("raw_sign_request") is not None:
            result["raw_sign_request"] = body.get("raw_sign_request")
        if body.get("sign_request") is not None:
            result["sign_request"] = body.get("sign_request")
        return result
    if spend_bundle_hex is None:
        return {
            "status": "skipped",
            "reason": "bundle_signer_missing_spend_bundle_hex",
            "operation_id": None,
        }
    return {
        "status": status,
