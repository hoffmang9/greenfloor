...
def execute_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    signer_request = {
        ...
        "chia_keys_dir": str(payload.get("chia_keys_dir", "")).strip(),
        "plan": {
            "op_type": op_type,
            ...
            "target_total_base_units": int(
                plan.get("target_total_base_units", size_base_units * op_count)
            ),
        },
    }

    signer_cmd = os.getenv("GREENFLOOR_CHIA_KEYS_SIGNER_CMD", "").strip()
    if not signer_cmd:
        signer_cmd = f"{sys.executable} -m greenfloor.cli.chia_keys_signer"

    try:
        completed = subprocess.run(
            shlex.split(signer_cmd),
            input=json.dumps(signer_request),
            capture_output=True,
            check=False,
            text=True,
            timeout=120,
        ...
        return {"status": "skipped", "reason": f"signer_spawn_error:{exc}", "operation_id": None}

    if completed.returncode != 0:
        err = completed.stderr.strip() or completed.stdout.strip() or "unknown_error"
        return {"status": "skipped", "reason": f"signer_failed:{err}", "operation_id": None}

    try:
        body = json.loads(completed.stdout.strip() or "{}")
    except json.JSONDecodeError:
        return {"status": "skipped", "reason": "signer_invalid_json", "operation_id": None}

    spend_bundle_hex = body.get("spend_bundle_hex")
    status = str(body.get("status", "executed"))
    reason = str(body.get("reason", "chia_keys_signer_success"))
    ...
    if spend_bundle_hex is None:
        return {
            "status": "skipped",
            "reason": "signer_missing_spend_bundle_hex",
            "operation_id": None,
        }
