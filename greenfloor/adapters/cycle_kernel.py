"""PyO3 bridge for the Rust daemon cycle kernel (strategy, lifecycle, dispatch)."""
...
def expand_strategy_actions(actions: list[dict[str, Any]]) -> list[dict[str, Any]]:
    signer = _import_signer()
    result = signer.expand_strategy_actions(actions)
    if not isinstance(result, list):
        raise TypeError("expand_strategy_actions returned non-list result")
    return [dict(item) for item in result]
...
def is_managed_upstream_transient_error(*, exception_class: str, error_text: str) -> bool:
    return bool(
        _import_signer().is_managed_upstream_transient_error(exception_class, error_text)
    )
...
def is_parallel_dispatch_transient_error(*, exception_class: str, error_text: str) -> bool:
    return bool(
        _import_signer().is_parallel_dispatch_transient_error(exception_class, error_text)
    )
...
def parallel_max_workers(*, submission_count: int, configured_max: int) -> int:
    return int(
        _import_signer().parallel_max_workers(int(submission_count), int(configured_max))
    )
...
def one_sided_offer_counts_by_side(
    ...
    signer = _import_signer()
    result = signer.one_sided_offer_counts_by_side(sell_counts, [int(size) for size in tracked_sizes])
    if not isinstance(result, dict):
...
def merge_market_cycle_strategy_execution(
    result: dict[str, Any],
    planned: int,
    executed: int,
) -> dict[str, Any]:
    signer = _import_signer()
    payload = signer.merge_market_cycle_strategy_execution(result, int(planned), int(executed))
    if not isinstance(payload, dict):
        raise TypeError("merge_market_cycle_strategy_execution returned non-dict result")
    return dict(payload)


def merge_market_cycle_cancel_policy(
    result: dict[str, Any],
    triggered: bool,
    planned: int,
    executed: int,
    ...
    signer = _import_signer()
    payload = signer.merge_market_cycle_cancel_policy(
        result,
        bool(triggered),
        int(planned),
        int(executed),
    ...
    if not isinstance(payload, dict):
        raise TypeError("merge_market_cycle_cancel_policy returned non-dict result")
    return dict(payload)


def record_market_cycle_phase_error(result: dict[str, Any]) -> dict[str, Any]:
    signer = _import_signer()
    payload = signer.record_market_cycle_phase_error(result)
    if not isinstance(payload, dict):
        raise TypeError("record_market_cycle_phase_error returned non-dict result")
    return dict(payload)
