...
def run_coin_split_step(
    *,
    ...
    spendable_scoped = filter_spendable_scoped_coins(
    ...
    if params.denomination_target is not None:
        split_gate = evaluate_coin_split_gate(
            ...
            resolved_asset_id=params.resolved_asset_id,
            size_base_units=int(params.denomination_target["size_base_units"]),
            required_count=int(params.denomination_target["required_count"]),
        )
    ...
    if params.denomination_target is not None:
        readiness_kwargs["required_min_count"] = int(params.denomination_target["required_count"])
    return CoinSplitStepResult(
...
def run_coin_combine_step(
    *,
    ...
    wallet_coins: list[dict[str, Any]],
) -> CoinOpIterationExecuteResult | CoinOpIterationEarlyExit:
    ...
    if params.explicit_coin_ids:
        ...
        if unresolved_coin_ids:
            return CoinOpIterationEarlyExit(
                return_code=2,
                unresolved_coin_ids=unresolved_coin_ids,
            )
        ...
        eligible_asset_coins = [
            ...
            and coin_meets_coin_op_min_amount(
                c, canonical_asset_id=params.combine_canonical_asset_id
            )
            and coin_matches_direct_spendable_lookup(
                wallet=params.wallet,
                coin=c,
                scoped_asset_id=params.resolved_asset_id,
                cache=direct_lookup_cache,
            )
            and str(c.get("id", "")).strip()
        ]
        if len(eligible_asset_coins) < params.number_of_coins:
            return CoinOpIterationEarlyExit(
                return_code=2,
        ...
        resolved_input_coin_ids = [
            str(coin.get("id", "")).strip()
            for coin in eligible_asset_coins[: params.number_of_coins]
        ]
    ...
    if params.denomination_target is not None:
        readiness_kwargs["max_allowed_count"] = int(
            params.denomination_target["combine_threshold_count"]
        )
    return CoinOpIterationExecuteResult(
        signature_request_id=signature_request_id,
